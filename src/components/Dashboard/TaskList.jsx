            ...
            >
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px', flex: 1, minWidth: 0 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <h3 style={{ fontSize: '16px', fontWeight: 600, color: isDone(task) ? 'var(--text-muted)' : 'var(--text-main)', textDecoration: isDone(task) ? 'line-through' : 'none', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', margin: 0, lineHeight: '24px' }}>
                    {task.title}
                  </h3>
                  ...
                  {(task.project || task.desc) && (
                    <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginTop: '4px' }}>
                      ...
                      {task.desc && (
                        <p style={{ fontSize: '13px', color: 'var(--text-muted)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', margin: 0 }}>
                          {task.desc}
                        </p>
                      )}
                    </div>
              ...
              <div className="task-row-meta" style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '6px', flexShrink: 0, minWidth: '90px' }}>
                {task.deadline && (() => {
                ...
                {task.recurrence && task.recurrence !== 'none' && (
                  <div style={{ fontSize: '11px', color: 'var(--accent-coral)', display: 'flex', alignItems: 'center', gap: '4px', fontWeight: 600 }}>
                    ...
                    <span style={{ textTransform: 'capitalize' }}>
                      {task.recurrence}
                      {task.recurrenceEndDate ? ` (до ${formatDisplayDate(task.recurrenceEndDate)})` : ''}
                    </span>
                  </div>
                )}
                {task.priority && (
